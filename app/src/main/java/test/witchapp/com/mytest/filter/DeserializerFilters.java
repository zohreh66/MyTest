...
public class DeserializerFilters implements JsonDeserializer<Filters> {
    ...
    protected static final String TAG_LABEL = "dataType";
            ...
            for (int i = 0; i < jArray.size(); i++) {
                ...
                if (jElement.isJsonObject()) {
                    ...
                    if (jObject.has(TAG_TYPE)) {
                        ...
                        if (FILTER_TYPE_CHECKBOX.equals(type)) {
                            final FilterTypeCheckBox filterTypeColor = new FilterTypeCheckBox();
                            filterTypeColor.setType(FILTER_TYPE_CHECKBOX);
...
//                            filterList.add(filterTypeRange);
     ...
     * @param filterType
     */
    protected void parseGeneralFields(JsonObject jObject, FilterType filterType) {
        ...
        if (jObject.has(TAG_LABEL))
            filterType.setDataType(jObject.get(TAG_LABEL).getAsString());
    }


    protected <T extends Object> List<T> parseTypeValues(Class<T> type, JsonObject jObject, JsonDeserializationContext context) {
        List<T> values = null;
        if (jObject.has(TAG_VALUES)) {
